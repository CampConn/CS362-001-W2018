...
public class Appt implements  Comparable<Appt>{
    ...
    private void isValid() {
        int NumDaysInMonth = CalendarUtil.NumDaysInMonth(startYear,startMonth); //bug: removed -1 from startMonth
    				
        if(startHour<0 || startHour>=23)    //bug: > not >= 23
    		this.valid=false;
    	else
        	if(startMinute<0 || startMinute>59)
            	...
            	if(startDay<1 || startDay>NumDaysInMonth)
                    ...
                    if(startMonth<0 || startMonth>12)   //bug: should start at 1, not 0
                		this.valid=false;
                	else
                		this.valid=true;
    }
    ...
    private String represntationApp(){
        String half = (getStartHour() > 11) ? "am" : "pm";  //bug: flip flopped am and pm
